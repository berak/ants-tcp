...
Ants.parseData = function (data) {
    ...
    for (var l = 0, llen = lines.length; l < llen; ++l) {
        ...
        if (tokens[0] === 'turn') {
            ...
            this.max_turns = turn;
            this.changes.push([]);
        } else if (tokens[0].toLowerCase() === 'm') {
...
Ants.viewTurn = function (turn) {
    var tessellate = false;
    if (tessellate) {
        var x_scale = Math.floor(this.x_scale / 2);
        var y_scale = Math.floor(this.y_scale / 2);
        var x_offset = Math.floor(this.canvas.width / 2 - this.width * x_scale);
        var y_offset = Math.floor(this.canvas.height / 2 - this.height * y_scale);
        var x_mid = this.width * x_scale + x_offset;
        var y_mid = this.height * y_scale + y_offset;
    } else {
        var x_offset = Math.floor(this.canvas.width / 2 - this.x_offset);
        var y_offset = Math.floor(this.canvas.height / 2 - this.y_offset);
    }
    for (var row = 0; row < this.height; ++row) {
        for (var col = 0; col < this.width; ++col) {
            ...
            if (tessellate) {
                var x = col * x_scale;
                var y = row * y_scale;
                this.dc.fillRect(x + x_offset, y + y_offset, x_scale, y_scale);
                this.dc.fillRect(x + x_mid,    y + y_offset, x_scale, y_scale);
                this.dc.fillRect(x + x_offset, y + y_mid,    x_scale, y_scale);
                this.dc.fillRect(x + x_mid,    y + y_mid,    x_scale, y_scale);
            } else {
...
var init = function () {
    // $('#replay_form')[0].reset();
    $.get(location.search + 'raw', function (response) {
